...
public class Main extends Canvas implements Runnable {
	...
	public void tick() {
		...
		if((b.getBallX() <= p.getX() + pWIDTH) && !(b.getBallX() <= 0)) {
			if(!(b.getBallY() + bHEIGHT < p.getY())
					&& !(b.getBallY() > p.getY() + pHEIGHT)) {
		...
		if((b.getBallX() + bWIDTH >= e.getEX())
				&& !(b.getBallX() + bWIDTH >= getWidth())) {
			if(!(b.getBallY() + bHEIGHT < e.getEY())
					&& !(b.getBallY() > e.getEY() + pHEIGHT)) {
				ballHit = (b.getBallY() + 4)
						- (e.getEY() + (pHEIGHT / 2) + 0.5);
				if(ballHit == 0) {
					b.setBallVelX(-(b.getBallVelX() + horizontalAccelerator));
				} else if(ballHit > 0) {
					if(b.getBallVelY() < 0) {
	...
	public void render() {
		...
		if(inMenu) {
			g.drawString("SinglePlayer", getWidth() / 2 - 20,
					getHeight() / 2 - 15);
			g.drawString("MultiPlayer", getWidth() / 2 - 20,
					getHeight() / 2 + 15);
		...
		if(!inMenu) {
			...
			g.drawString(s, (getWidth() / 2)
					- (s.length() * g.getFont().getSize()) / 4, 20);
		}
		// ////////////////////////////////
		g.dispose();
		bs.show();
